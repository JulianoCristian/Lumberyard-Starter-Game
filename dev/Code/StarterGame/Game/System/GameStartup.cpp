...
int GameStartup::Run(const char* autoStartLevelName)
{
    ...
    {
		// we want the cursor to be visible by default, so try showing it
		if (ShowCursor(TRUE) > 0)
		{
			// if the count returned is > 0, the cursor was already visible, so undo the action to get the display count to match HardwareMouse's logic
			ShowCursor(FALSE);
		}
