...
function weapon:OnActivate(parent, entityId, properties)
	...
	self.activateEventId = GameplayNotificationId(self.entityId, self.Properties.Events.Activate);
	self.activateHandler = GameplayNotificationBus.Connect(self, self.activateEventId);
	self.fireEventId = GameplayNotificationId(self.entityId, self.Properties.Events.Fire);
	self.fireHandler = GameplayNotificationBus.Connect(self, self.fireEventId);
	self.reloadEventId = GameplayNotificationId(self.entityId, self.Properties.Events.Reload);
	self.reloadHandler = GameplayNotificationBus.Connect(self, self.reloadEventId);
	self.useWeaponForwardForAimingEventId = GameplayNotificationId(self.entityId, self.Properties.Events.UseWeaponForwardForAiming);
	...
	self.FireSuccessEventId = GameplayNotificationId(self.Properties.Owner, self.Properties.Events.FireSuccess);
	self.FireFailEventId = GameplayNotificationId(self.Properties.Owner, self.Properties.Events.FireFail);
	self.hideCrosshairEventId = GameplayNotificationId(self.entityId, self.Properties.Events.HideCrosshair);
