...
function particlespawnrequester:OnActivate()
	...
	self.activationEventId = GameplayNotificationId(self.entityId, self.Properties.ActivationEvent);
...
function particlespawnrequester:OnTick(deltaTime, timePoint)
	...
	if (self.performedFirstUpdate == false) then
		...
		self.eventId = GameplayNotificationId(particleManager, "SpawnParticleEvent");
		self:ResetSpawner();
