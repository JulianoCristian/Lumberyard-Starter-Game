...
function UIMapTrackable:OnActivate()
	...
	self.EnableUIEventId = GameplayNotificationId(EntityId(), self.Properties.Messages.EnableUIEvent);
	self.EnableUIHandler = GameplayNotificationBus.Connect(self, self.EnableUIEventId);
	self.HideUIEventId = GameplayNotificationId(EntityId(), self.Properties.Messages.HideUIEvent);
	self.HideUIHandler = GameplayNotificationBus.Connect(self, self.HideUIEventId);
	self.ShowMarkerEventId = GameplayNotificationId(self.entityId, self.Properties.Messages.ShowMarkerEvent);
	self.ShowMarkerHandler = GameplayNotificationBus.Connect(self, self.ShowMarkerEventId);
	self.SetTrackerEventId = GameplayNotificationId(self.entityId, self.Properties.Messages.SetTrackerEvent);
...
function UIMapTrackable:OnTick(deltaTime, timePoint)
	...
	if (not self.Properties.Tracker:IsValid()) then
		if(self.Properties.TrackerTags ~= "") then
			...
			if (trackerProspect ~= nil and trackerProspect:IsValid()) then 
