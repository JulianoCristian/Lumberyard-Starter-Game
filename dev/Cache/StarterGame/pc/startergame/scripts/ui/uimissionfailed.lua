...
function uimissionfailed:OnActivate()
	...
	self.ShowScreenEventId = GameplayNotificationId(self.entityId, self.Properties.ShowScreenEvent);
...
function uimissionfailed:CloseScreen()
	...
	--local chEventId = GameplayNotificationId(ch, "EnableCrosshairEvent");
...
function uimissionfailed:OnTick(deltaTime, timePoint)
	...
	local turnCamera = GameplayNotificationId(playerCam, "LookLeftRightForced");
	...
	if(self.timer < 0) then
		if (self.Properties.AutoFinish == true) then
			...
			self.anyButtonEventId = GameplayNotificationId(self.entityId, "AnyKey");
...
function uimissionfailed:SetControls(tag, event, enabled)
	...
	local controlsEventId = GameplayNotificationId(entity, event);
