...
function weaponswitcher:OnActivate()    
	...
	self.weaponSwitchEventId = GameplayNotificationId(self.entityId, "WeaponSwitch");
...
function weaponswitcher:SetupNewWeapon()
	...
	self.activeWeaponActivateEventId = GameplayNotificationId(self.activeWeapon.Weapon, self.Properties.Weapons.GenericEvents.Activate);

	GameplayNotificationBus.Event.OnEventBegin(self.activeWeaponActivateEventId, 1);
	
	local onSetupNewWeaponEventId = GameplayNotificationId(self.entityId, "OnSetupNewWeapon");
...
function weaponswitcher:OnEventBegin(value)
	-- Player controls.
	if (GameplayNotificationBus.GetCurrentBusId() == self.weaponSwitchEventId) then
		self:ChangeWeapon();
	end
