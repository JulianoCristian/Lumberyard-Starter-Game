...
function doublesidedtriggerreceiver:OnActivate()
	...
	self.onEnterLeftEventId = GameplayNotificationId(self.entityId, self.Properties.EventsToReceive.OnEnterLeft);
	self.onEnterLeftEventHandler = GameplayNotificationBus.Connect(self, self.onEnterLeftEventId);
	self.onEnterRightEventId = GameplayNotificationId(self.entityId, self.Properties.EventsToReceive.OnEnterRight);
	self.onEnterRightEventHandler = GameplayNotificationBus.Connect(self, self.onEnterRightEventId);
	-- LeftToX
	self.onExitLeftToLeftEventId = GameplayNotificationId(self.entityId, self.Properties.EventsToReceive.OnExitLeftToLeft);
	self.onExitLeftToLeftEventHandler = GameplayNotificationBus.Connect(self, self.onExitLeftToLeftEventId);
	self.onExitLeftToRightEventId = GameplayNotificationId(self.entityId, self.Properties.EventsToReceive.OnExitLeftToRight);
	self.onExitLeftToRightEventHandler = GameplayNotificationBus.Connect(self, self.onExitLeftToRightEventId);
	-- RightToX
	self.onExitRightToLeftEventId = GameplayNotificationId(self.entityId, self.Properties.EventsToReceive.OnExitRightToLeft);
	self.onExitRightToLeftEventHandler = GameplayNotificationBus.Connect(self, self.onExitRightToLeftEventId);
	self.onExitRightToRightEventId = GameplayNotificationId(self.entityId, self.Properties.EventsToReceive.OnExitRightToRight);
