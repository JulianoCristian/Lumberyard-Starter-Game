...
function doublesidedtrigger:OnActivate()
	...
	self.onEnterLeftEventId = GameplayNotificationId(self.entityId, self.Properties.Side1.EventName);
	self.onEnterLeftEventHandler = GameplayNotificationBus.Connect(self, self.onEnterLeftEventId);
	self.onExitLeftEventId = GameplayNotificationId(self.entityId, self.Properties.Side1.EventNameExit);
	self.onExitLeftEventHandler = GameplayNotificationBus.Connect(self, self.onExitLeftEventId);
	self.onEnterRightEventId = GameplayNotificationId(self.entityId, self.Properties.Side2.EventName);
	self.onEnterRightEventHandler = GameplayNotificationBus.Connect(self, self.onEnterRightEventId);
	self.onExitRightEventId = GameplayNotificationId(self.entityId, self.Properties.Side2.EventNameExit);
...
function doublesidedtrigger:SendEventToThisEntity(enteringEntityId, eventName)
	...
	local notificationId = GameplayNotificationId(receiverId, eventName);
