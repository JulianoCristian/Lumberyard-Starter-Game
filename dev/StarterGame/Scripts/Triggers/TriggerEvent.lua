...
function triggerevent:SendTheEvent(enteringEntityId)
	...
	-- Send the event to the list of entities, if a valid one has been selected.
	if (self.Properties.Recipients.Entities ~= -1) then
		local size = ObjectListerComponentRequestsBus.Event.GetListSize(self.entityId, self.Properties.Recipients.Entities);
		for i = 0, size - 1 do
			local obj = ObjectListerComponentRequestsBus.Event.GetListItem(self.entityId, self.Properties.Recipients.Entities, i);
			if (self:EntityDoesNotContainAnExcludedTag(obj.entityId)) then
				self:SendTheEventToThisEntity(enteringEntityId, obj.entityId);
			end
		end
	end
...
function triggerevent:SendTheEventToThisEntity(enteringEntityId, entityId)
	...
	local notificationId = GameplayNotificationId(entityId, self.EventNameToUse);
