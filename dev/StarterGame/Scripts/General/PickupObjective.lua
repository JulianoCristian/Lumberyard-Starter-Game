...
function pickupobjective:OnTriggerAreaEntered(enteringEntityId)
	...
	local notificationId = GameplayNotificationId(enteringEntityId, self.Properties.EventName);
	GameplayNotificationBus.Event.OnEventBegin(notificationId, self.Properties.ObjectiveID);
	local markernotificationId = GameplayNotificationId(self.entityId, self.Properties.ShowMarkerMessage);
	GameplayNotificationBus.Event.OnEventBegin(markernotificationId, 0);
	local statusnotificationId = GameplayNotificationId(self.entityId, self.Properties.SetStatusMessage);
