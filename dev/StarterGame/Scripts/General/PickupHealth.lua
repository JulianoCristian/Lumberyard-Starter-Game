...
function pickuphealth:OnActivate()
	...
	self.EntityInquiryReturnEventId = GameplayNotificationId(self.entityId, self.Properties.EventInquiryReturnName);
...
function pickuphealth:OnEventBegin(value)
	...
	if ((GameplayNotificationBus.GetCurrentBusId() == self.EntityInquiryReturnEventId) and (value > 0)) then
		...
		-- award the health
		local notificationId = GameplayNotificationId(self.enteringEntityId, self.Properties.EventName);
...
function pickuphealth:OnTriggerAreaEntered(enteringEntityId)
	...
	local notificationId = GameplayNotificationId(self.enteringEntityId, self.Properties.EventInquiryName);
