...
namespace StarterGameGem
{
	...
	void StatComponent::Activate()
	{
		...
		m_setMaxEventID	= AZ::GameplayNotificationId(myID, AZ::Crc32(m_setMaxEventName.c_str()));
		m_setMinEventID	= AZ::GameplayNotificationId(myID, AZ::Crc32(m_setMinEventName.c_str()));
		m_setRegenSpeedEventID = AZ::GameplayNotificationId(myID, AZ::Crc32(m_setRegenSpeedEventName.c_str()));
		m_setRegenDelayEventID = AZ::GameplayNotificationId(myID, AZ::Crc32(m_setRegenDelayEventName.c_str()));
		m_setValueEventID = AZ::GameplayNotificationId(myID, AZ::Crc32(m_setValueEventName.c_str()));
		m_deltaValueEventID = AZ::GameplayNotificationId(myID, AZ::Crc32(m_deltaValueEventName.c_str()));
		m_resetEventID = AZ::GameplayNotificationId(myID, AZ::Crc32(m_resetEventName.c_str()));
		m_GetValueEventID = AZ::GameplayNotificationId(myID, AZ::Crc32(m_GetValueEventName.c_str()));
		m_GetUsedCapacityEventID = AZ::GameplayNotificationId(myID, AZ::Crc32(m_GetUsedCapacityEventName.c_str()));
		m_GetUnUsedCapacityEventID = AZ::GameplayNotificationId(myID, AZ::Crc32(m_GetUnUsedCapacityEventName.c_str()));
		
		//CryLog("StatComponent::Activate.  [%llu] \"%s\" combinedID == %s", (AZ::u64)myID, m_deltaValueEventName.c_str(), m_deltaValueEventID.ToString().c_str());
	...
	void StatComponent::ValueChanged(bool resetTimer/* = true*/, bool regenBegin /*= false*/)
	{
		...
		AZ::EntityId myID = GetEntityId();
		//CryLog("StatComponent::ValueChanged to \"%.03f\" on %llu", m_value, (AZ::u64)myID);

		AZStd::any paramToBus(m_value);

		// send any messages that are relevent for the current state of my value
		{	// changed
			AZ::GameplayNotificationId gameplayId = AZ::GameplayNotificationId(myID, m_valueChangedEventName.c_str());
			AZ::GameplayNotificationBus::Event(gameplayId, &AZ::GameplayNotificationBus::Events::OnEventBegin, paramToBus);
		}

		if (regenStarted)
		{	// regen begin
			AZ::GameplayNotificationId gameplayId = AZ::GameplayNotificationId(myID, m_regenStartEventName.c_str());
			AZ::GameplayNotificationBus::Event(gameplayId, &AZ::GameplayNotificationBus::Events::OnEventBegin, paramToBus);
		}

		if (regenStopped)
		{	// changed
			AZ::GameplayNotificationId gameplayId = AZ::GameplayNotificationId(myID, m_regenEndEventName.c_str());
			AZ::GameplayNotificationBus::Event(gameplayId, &AZ::GameplayNotificationBus::Events::OnEventBegin, paramToBus);
		}

		if (m_value == m_min)
		{
			AZ::GameplayNotificationId gameplayId = AZ::GameplayNotificationId(myID, m_emptyEventName.c_str());
			AZ::GameplayNotificationBus::Event(gameplayId, &AZ::GameplayNotificationBus::Events::OnEventBegin, paramToBus);
		}

		if (m_value == m_max)
		{
			AZ::GameplayNotificationId gameplayId = AZ::GameplayNotificationId(myID, m_fullEventName.c_str());
			AZ::GameplayNotificationBus::Event(gameplayId, &AZ::GameplayNotificationBus::Events::OnEventBegin, paramToBus);
		}
	}
	...
	void StatComponent::OnEventBegin(const AZStd::any& value)
	{
		...
		AZ::EntityId myID = GetEntityId();
		//CryLog("StatComponent::OnEventBegin. [%llu], is a number %d : %.03f", (AZ::u64)myID, isNumber, valueAsFloat);
		...
		else if (*AZ::GameplayNotificationBus::GetCurrentBusId() == m_GetValueEventID)
		{
			...
			AZ::GameplayNotificationId gameplayId = AZ::GameplayNotificationId(valueAsEntityID, m_SendValueEventName.c_str());
			AZ::GameplayNotificationBus::Event(gameplayId, &AZ::GameplayNotificationBus::Events::OnEventBegin, paramToBus);
		}
		else if (*AZ::GameplayNotificationBus::GetCurrentBusId() == m_GetUsedCapacityEventID)
		{
			AZStd::any paramToBus(m_value - m_min);
			AZ::GameplayNotificationId gameplayId = AZ::GameplayNotificationId(valueAsEntityID, m_SendUsedCapacityEventName.c_str());
			AZ::GameplayNotificationBus::Event(gameplayId, &AZ::GameplayNotificationBus::Events::OnEventBegin, paramToBus);
		}
		else if (*AZ::GameplayNotificationBus::GetCurrentBusId() == m_GetUnUsedCapacityEventID)
		{
			AZStd::any paramToBus(m_max - m_value);
			AZ::GameplayNotificationId gameplayId = AZ::GameplayNotificationId(valueAsEntityID, m_SendUnUsedCapacityEventName.c_str());
			AZ::GameplayNotificationBus::Event(gameplayId, &AZ::GameplayNotificationBus::Events::OnEventBegin, paramToBus);
		}
	}
