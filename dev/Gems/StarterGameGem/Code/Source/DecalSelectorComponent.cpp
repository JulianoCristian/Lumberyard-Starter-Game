...
namespace StarterGameGem
{
	...
	void DecalSelectorComponent::Reflect(AZ::ReflectContext* reflection)
	{
		...
		if (AZ::BehaviorContext* behaviour = azrtti_cast<AZ::BehaviorContext*>(reflection))
		{
			behaviour->EBus<DecalSelectorComponentRequestsBus>("DecalSelectorComponentRequestsBus")
	...
	void DecalSelectorComponent::SpawnDecal(const ParticleSpawnerParams& params)
	{
		...
		if (!entityId.IsValid())
		{
			...
			CryLog("DecalSelectorComponent: We received an invalid entity ID from a decal pool.");
			return;
		}
	...
	void DecalSelectorComponent::AddDecalToPool(AZ::EntityId decal, int surfaceIndex)
	{
		...
		{
			CryLog("DecalSelectorComponent: Something attempted to add a decal to a pool that shouldn't exist.");
			return;
		}
